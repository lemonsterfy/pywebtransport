...
- `connection_cleanup_interval` (`float`): Interval for the task that cleans up closed connections. Default: `30.0`.
...
- `stream_cleanup_interval` (`float`): Interval for the task that cleans up closed streams. Default: `15.0`.
...
- `ca_certs` (`str | None`): Path to a file of concatenated CA certificates.
- `certfile` (`str | None`): Path to the client's certificate file (for mTLS).
- `keyfile` (`str | None`): Path to the client's private key file (for mTLS).
...
- **`create_for_production(*, ca_certs: str | None = None, ...) -> ClientConfig`**: Creates a config optimized for production with stricter security and performance settings.
...
- **`merge(other: ClientConfig | dict[str, Any]) -> ClientConfig`**: Merges the current config with another, returning a new instance.
...
- `connection_idle_timeout` (`float`): Time in seconds after which an idle connection is closed. Default: `60.0`.
- `connection_idle_check_interval` (`float`): Interval for checking for idle connections. Default: `5.0`.
- `session_cleanup_interval` (`float`): Interval for the task that cleans up closed sessions. Default: `60.0`.
- `stream_cleanup_interval` (`float`): Interval for the task that cleans up closed streams. Default: `15.0`.
...
- `ca_certs` (`str | None`): Path to CA certificates for client certificate validation (mTLS).
...
- **`validate() -> None`**: Validates the configuration values, raising `ConfigurationError` on failure.
- **`copy() -> ServerConfig`**: Returns a deep copy of the configuration instance.
- **`update(**kwargs: Any) -> ServerConfig`**: Returns a new `ServerConfig` instance with updated values.
- **`merge(other: ServerConfig | dict[str, Any]) -> ServerConfig`**: Merges the current config with another, returning a new instance.
- **`to_dict() -> dict[str, Any]`**: Converts the configuration to a dictionary.
...
- **`ConfigBuilder(config_type: str = "client")`**: Initializes the builder. `config_type` can be `"client"` or `"server"`.
...
- **`build() -> ClientConfig | ServerConfig`**: Constructs and returns the final configuration object.
