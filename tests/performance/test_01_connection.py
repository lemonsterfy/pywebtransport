...
class TestConnectionPerformance:
    ...
    def test_connection_establishment_latency(
        ...
        async def connect_and_close() -> None:
            async with WebTransportClient.create(config=default_client_config) as client:
                session = await client.connect(f"{SERVER_URL}{CONNECTION_TEST_ENDPOINT}")
                await session.close()
    ...
    def test_high_concurrency_throughput(
        ...
        async def single_client_task(config: ClientConfig) -> bool:
            try:
                ...
                logger.warning(f"A concurrent connection task failed: {e}")
                return False
            except Exception as e:
                logger.error(f"Unexpected error in concurrent task: {e}", exc_info=True)
                return False

        async def connect_all_concurrently() -> int:
            tasks = [
                asyncio.create_task(single_client_task(concurrency_client_config)) for _ in range(CONCURRENCY_LEVEL)
            ]
        ...
        logger.info(f"Concurrency Test Iteration: {successful_connections}/{CONCURRENCY_LEVEL} connections succeeded.")
        assert (
        ...
        logger.info(f"Connection Throughput: {connections_per_second:.2f} connections/sec")
