...
class TestServerMonitor:
    ...
    def test_initialization(self, mock_server: Any, interval: float) -> None:
        monitor = ServerMonitor(server=mock_server, monitoring_interval=interval)
    ...
    def test_create_factory_method(self, mock_server: Any) -> None:
        monitor = ServerMonitor.create(server=mock_server, monitoring_interval=42.0)
    ...
    def test_is_monitoring_property(self, mocker: MockerFixture, mock_server: Any) -> None:
        monitor = ServerMonitor(server=mock_server)
    ...
    async def test_async_context_manager(self, mocker: MockerFixture, mock_server: Any, mock_logger: Any) -> None:
        ...
        async with ServerMonitor(server=mock_server) as monitor:
            assert monitor.is_monitoring
    ...
    async def test_aenter_handles_runtime_error(
        ...
        mock_logger.error.assert_called_once_with(
            "Failed to start server monitor: No running event loop.", exc_info=True
        )
    ...
    async def test_monitor_loop_logic(self, mocker: MockerFixture, mock_server: Any, mock_logger: Any) -> None:
        monitor = ServerMonitor(server=mock_server, monitoring_interval=0.01)
    ...
    async def test_monitor_loop_exception_handling(
        ...
        mock_logger.error.assert_called_once_with(
            "Server monitor loop encountered a critical error: %s", error, exc_info=True
        )
    ...
    async def test_check_for_alerts_generation_and_deduplication(
        ...
        mock_logger.warning.assert_called_once_with("Health Alert: %s - %s", "unhealthy", "Server is not serving.")
        ...
        mock_logger.warning.assert_called_with("Health Alert: %s - %s", "degraded", "Low success rate.")
    ...
    async def test_check_for_alerts_handles_exception(
        ...
        mock_logger.error.assert_called_once_with("Alert check failed: %s", error, exc_info=True)
