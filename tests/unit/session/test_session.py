...
class TestWebTransportSession:
    ...
    async def test_sync_protocol_state_not_connected(
        self, mock_connection: Any, mock_protocol_handler: Any, mocker: MockerFixture
    ) -> None:
        mock_session_info = mocker.MagicMock()
    ...
    async def test_datagrams_property_lazy_creation(
        ...
        assert hasattr(session, "_datagram_transport")
        assert session._datagram_transport is not None
        cast(mock.AsyncMock, datagram_transport.initialize).assert_awaited_once()

    async def test_pubsub_property_lazy_creation_and_idempotent(
        self, session: WebTransportSession, mocker: MockerFixture
    ) -> None:
        mock_pubsub_manager_class = mocker.patch("pywebtransport.pubsub.manager.PubSubManager", autospec=True)
        assert not hasattr(session, "_pubsub_manager")

        pubsub_manager1 = session.pubsub
        pubsub_manager2 = session.pubsub

        mock_pubsub_manager_class.assert_called_once_with(session=session)
        assert pubsub_manager1 is mock_pubsub_manager_class.return_value
        assert pubsub_manager2 is pubsub_manager1
        assert hasattr(session, "_pubsub_manager")

    async def test_rpc_property_lazy_creation_and_idempotent(
        self, session: WebTransportSession, mocker: MockerFixture
    ) -> None:
        mock_rpc_manager_class = mocker.patch("pywebtransport.rpc.manager.RpcManager", autospec=True)
        assert not hasattr(session, "_rpc_manager")
    ...
    async def test_close_with_active_managers(self, session: WebTransportSession, mocker: MockerFixture) -> None:
        mock_pubsub_close = mocker.patch("pywebtransport.pubsub.manager.PubSubManager.close", new_callable=AsyncMock)
        mock_rpc_close = mocker.patch("pywebtransport.rpc.manager.RpcManager.close", new_callable=AsyncMock)
        _ = session.pubsub
        _ = session.rpc

        await session.close(close_connection=False)

        mock_pubsub_close.assert_awaited_once()
        mock_rpc_close.assert_awaited_once()
    ...
    @pytest.mark.parametrize(
        "direction", [StreamDirection.BIDIRECTIONAL, StreamDirection.RECEIVE_ONLY, StreamDirection.SEND_ONLY]
    )
    ...
    async def test_on_stream_opened(
        ...
        event_data: dict[str, Any] = {"session_id": "session-1", "stream_id": 2, "direction": direction}
        if with_payload:
    ...
    async def test_debug_state_no_datagrams(self, session: WebTransportSession) -> None:
        assert not hasattr(session, "_datagram_transport")
        debug_info = await session.debug_state()
    ...
    @pytest.mark.parametrize("connection_setup", ["normal", "no_connection", "no_info_attr", "no_last_activity_attr"])
    async def test_monitor_health(
        ...
        if connection_setup == "no_connection":
            ...
            assert session.connection
            mock_info = mocker.MagicMock(spec=[])
            mocker.patch.object(WebTransportConnection, "info", new_callable=PropertyMock, return_value=mock_info)
    ...
    async def test_on_session_closed_remotely_when_already_closing_or_closed(
        self, session: WebTransportSession, mocker: MockerFixture, initial_state: SessionState
    ) -> None:
        close_mock = mocker.patch.object(session, "close", new_callable=mocker.AsyncMock)
    ...
    @pytest.mark.parametrize(
        "state, stats, connection_setup, expected_issue",
        [
            (SessionState.CONNECTING, {}, "connected", "Session stuck in connecting state"),
            (SessionState.CONNECTED, {"streams_created": 100, "stream_errors": 15}, "connected", "High error rate"),
            (SessionState.CONNECTED, {}, "not_connected", "Underlying connection not available"),
            (SessionState.CONNECTED, {}, "no_connection", "Underlying connection not available"),
        ],
    )
    async def test_diagnose_issues(
        ...
        if connection_setup == "no_connection":
            ...
            is_connected = connection_setup == "connected"
            mocker.patch.object(
                type(session.connection), "is_connected", new_callable=PropertyMock, return_value=is_connected
            )

        issues = await session.diagnose_issues()
        assert any(expected_issue in issue for issue in issues)

    async def test_diagnose_issues_stale_session(self, session: WebTransportSession, mocker: MockerFixture) -> None:
        session._state = SessionState.CONNECTED
        mocker.patch.object(
            session,
            "get_session_stats",
            return_value={"uptime": 4000, "active_streams": 0, "streams_created": 10},
        )
        issues = await session.diagnose_issues()
        assert any("Session appears stale" in issue for issue in issues)
    ...
    async def test_close_handles_multiple_resource_errors(
        self, session: WebTransportSession, mocker: MockerFixture
    ) -> None:
        datagram_transport = await session.datagrams
        error1 = ValueError("Streams failed")
        error2 = RuntimeError("Datagrams failed")
        assert session.stream_manager is not None
        cast(AsyncMock, session.stream_manager.shutdown).side_effect = error1
        cast(AsyncMock, datagram_transport.close).side_effect = error2
        with pytest.raises(ExceptionGroup) as excinfo:
            await session.close(close_connection=False)
        assert session.is_closed
        assert excinfo.value.exceptions == (error1, error2)

    async def test_close_connection_error_after_task_group(
        self, session: WebTransportSession, mock_connection: Any
    ) -> None:
        error = ValueError("Connection failed")
        cast(AsyncMock, mock_connection.close).side_effect = error
        with pytest.raises(ValueError, match="Connection failed"):
            await session.close(close_connection=True)
        assert session.is_closed

    async def test_close_handles_all_resource_errors(self, session: WebTransportSession, mock_connection: Any) -> None:
        datagram_transport = await session.datagrams
        error_stream = ValueError("Streams failed")
        error_datagram = RuntimeError("Datagrams failed")
        error_connection = IOError("Connection failed")
        assert session.stream_manager is not None
        cast(AsyncMock, session.stream_manager.shutdown).side_effect = error_stream
        cast(AsyncMock, datagram_transport.close).side_effect = error_datagram
        cast(AsyncMock, mock_connection.close).side_effect = error_connection
        with pytest.raises(ExceptionGroup) as excinfo:
            await session.close(close_connection=True)
        assert session.is_closed
        assert excinfo.value.exceptions == (error_stream, error_datagram)
