...
from pywebtransport import ClientConfig, Event, ProtocolError, ServerConfig, constants
...
def mock_pylsqpack(mocker: MagicMock) -> dict[str, MagicMock]:
    ...
    mock_encoder_instance.encode.return_value = (
        b"encoder-stream-bytes",
        b"headers-payload",
    )
    ...
    mock_decoder_instance.feed_header.return_value = (
...
class TestWebTransportH3EngineEventHandling:
    def test_get_server_name(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        assert engine.get_server_name() == "test.server"

    @pytest.mark.asyncio
    async def test_bidi_stream_without_wt_frame_is_regular_h3(
        self,
        mock_quic: MagicMock,
        ...
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"headers-payload")
        ...
        assert stream.stream_type is None
        assert stream.control_stream_id is None

    @pytest.mark.asyncio
    async def test_draining_uni_stream_is_ignored(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        stream = engine._get_or_create_stream(stream_id=CLIENT_UNI_STREAM_ID)
    ...
    @pytest.mark.asyncio
    async def test_empty_unblocked_streams_does_nothing(
        self,
        mock_quic: MagicMock,
        mock_pylsqpack: dict[str, MagicMock],
        client_config: ClientConfig,
        ...
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        mock_pylsqpack["decoder_instance"].feed_encoder.return_value = []
    ...
    async def test_handle_bidi_stream_parses_capsules_after_headers(
        ...
        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"encoded-headers")
    ...
    async def test_handle_bidi_webtransport_setup_and_data(
        ...
        assert h3_events[0].control_stream_id == 789
    ...
    async def test_handle_control_stream_emits_settings_event(
        ...
        settings_frame = _encode_frame(
            frame_type=constants.H3_FRAME_TYPE_SETTINGS,
            frame_data=_encode_settings(settings=settings),
        )
    ...
    async def test_handle_qpack_stream_blocked_and_resumed(
        ...
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"blocked-headers")
        event = StreamDataReceived(data=headers_frame, stream_id=CLIENT_BIDI_STREAM_ID, end_stream=False)
    ...
    async def test_handle_uni_webtransport_setup_and_data(
        ...
        assert len(h3_events) == 1
        assert isinstance(h3_events[0], WebTransportStreamDataReceived)
        assert h3_events[0].control_stream_id == 101
    ...
    @pytest.mark.asyncio
    async def test_headers_on_draining_stream_is_ignored(
        self,
        mock_quic: MagicMock,
        mock_pylsqpack: dict[str, MagicMock],
        client_config: ClientConfig,
        ...
        stream.is_draining = True
        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"ignored-headers")
        event = StreamDataReceived(data=headers_frame, stream_id=CLIENT_BIDI_STREAM_ID, end_stream=False)
    ...
    @pytest.mark.parametrize(
        "partial_data",
        [b"\x41", encode_uint_var(constants.H3_FRAME_TYPE_WEBTRANSPORT_STREAM) + b"\x9f"],
    )
    @pytest.mark.asyncio
    async def test_partial_bidi_webtransport_frame(
        ...
        assert not h3_events
        assert b"".join(engine._get_or_create_stream(stream_id=CLIENT_BIDI_STREAM_ID).buffer) == partial_data

    @pytest.mark.asyncio
    async def test_partial_non_data_frame(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"some-header-payload")
        event = StreamDataReceived(data=headers_frame[:-1], stream_id=CLIENT_BIDI_STREAM_ID, end_stream=False)

        h3_events = await engine.handle_event(event=event)

        assert not h3_events
        assert b"".join(engine._get_or_create_stream(stream_id=CLIENT_BIDI_STREAM_ID).buffer) == b"some-header-payloa"

    @pytest.mark.asyncio
    async def test_partial_uni_stream_type(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        event = StreamDataReceived(data=b"\xc0", stream_id=SERVER_UNI_STREAM_ID, end_stream=False)
        h3_events = await engine.handle_event(event=event)
        assert not h3_events
        mock_quic.close.assert_not_called()
    ...
    async def test_partial_uni_webtransport_data(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        assert h3_events3[0].control_stream_id == 101
    ...
    async def test_partial_uni_webtransport_frame(
        ...
        assert not h3_events
        assert b"".join(engine._get_or_create_stream(stream_id=CLIENT_UNI_STREAM_ID).buffer) == expected_buffer
    ...
    async def test_receive_data_on_blocked_stream(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        assert not h3_events
        assert b"".join(stream.buffer) == b"some data"

    @pytest.mark.asyncio
    async def test_resume_blocked_stream_with_buffered_data(
        self,
        mock_quic: MagicMock,
        mock_pylsqpack: dict[str, MagicMock],
        client_config: ClientConfig,
        ...
        capsule_data = _encode_frame(frame_type=0x41, frame_data=b"test")
        stream.buffer.append(capsule_data)
        mock_pylsqpack["decoder_instance"].feed_encoder.return_value = [CLIENT_BIDI_STREAM_ID]
        mock_pylsqpack["decoder_instance"].resume_header.return_value = (
            b"",
            [(b":status", b"200")],
        )
        qpack_data = encode_uint_var(constants.H3_STREAM_TYPE_QPACK_ENCODER) + b"qpack-unblock-data"
        ...
        assert any(isinstance(e, HeadersReceived) for e in h3_events)
        assert any(isinstance(e, CapsuleReceived) for e in h3_events)

    @pytest.mark.asyncio
    async def test_resume_header_on_unblocked_stream(
        self,
        mock_quic: MagicMock,
        mock_pylsqpack: dict[str, MagicMock],
        client_config: ClientConfig,
        ...
        assert len(h3_events) == 1
        assert isinstance(h3_events[0], HeadersReceived)
        mock_pylsqpack["decoder_instance"].resume_header.assert_called_with(CLIENT_BIDI_STREAM_ID)
...
class TestWebTransportH3EngineHeaderValidation:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [(b"regular", b"header"), (b":method", b"GET")],
                "is not allowed after regular headers",
            ),
            ...
            ([(b"key", b"invalid\x00value")], "has forbidden characters"),
            (
                [(b":method", b"GET"), (b":scheme", b"http"), (b":authority", b"")],
                "cannot be empty",
            ),
            (
                [
    ...
    async def test_invalid_request_headers(
        ...
        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"bad-headers")
        event = StreamDataReceived(data=headers_frame, stream_id=CLIENT_BIDI_STREAM_ID, end_stream=False)
...
class TestWebTransportH3EngineInitialization:
    ...
    def test_init_sends_correct_settings_client(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        settings = _parse_settings(data=settings_bytes)
    ...
    def test_init_server_mode(self, mock_quic_server: MagicMock, server_config: ServerConfig) -> None:
        ...
        assert engine.is_client is False
...
class TestWebTransportH3EngineMiscErrors:
    ...
    async def test_malformed_settings_frame_fails(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        settings_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_SETTINGS, frame_data=b"\x06")
...
class TestWebTransportH3EngineProtocolErrors:
    ...
    async def test_data_before_headers_fails(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        data_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_DATA, frame_data=b"some data")
    ...
    async def test_duplicate_setting_fails(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        buf += encode_uint_var(constants.SETTINGS_H3_DATAGRAM) + encode_uint_var(1)
        settings_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_SETTINGS, frame_data=buf)
        control_data = encode_uint_var(constants.H3_STREAM_TYPE_CONTROL) + settings_frame
        event = StreamDataReceived(data=control_data, stream_id=SERVER_UNI_STREAM_ID, end_stream=False)
    ...
    async def test_first_control_frame_not_settings_fails(
        ...
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        data_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_DATA, frame_data=b"wrong frame")
    ...
    async def test_headers_on_control_stream_fails(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        settings_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_SETTINGS, frame_data=b"")
        control_data = encode_uint_var(constants.H3_STREAM_TYPE_CONTROL) + settings_frame
        event1 = StreamDataReceived(data=control_data, stream_id=SERVER_UNI_STREAM_ID, end_stream=False)
        await engine.handle_event(event=event1)

        headers_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {
                    constants.SETTINGS_WT_MAX_SESSIONS: 1,
                    constants.SETTINGS_H3_DATAGRAM: 0,
                },
                "WT_MAX_SESSIONS requires H3_DATAGRAM",
            ),
            ({constants.SETTINGS_ENABLE_CONNECT_PROTOCOL: 2}, "setting must be 1 if present"),
            ({0x02: 1}, "Setting identifier 0x2 is reserved"),
        ],
    )
    async def test_invalid_setting_fails(
        self,
        mock_quic: MagicMock,
        client_config: ClientConfig,
        ...
        mock_quic.close.assert_called_once()
        assert error_match in mock_quic.close.call_args.kwargs["reason_phrase"]
        assert mock_quic.close.call_args.kwargs["error_code"] == ErrorCodes.H3_SETTINGS_ERROR
    ...
    async def test_invalid_frame_on_capsule_stream(
        self, mock_quic: MagicMock, client_config: ClientConfig, frame_type: int
    ) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        stream = engine._get_or_create_stream(stream_id=CLIENT_BIDI_STREAM_ID)
        stream.headers_recv_state = _HeadersState.AFTER_HEADERS
        capsule_header = encode_uint_var(frame_type) + encode_uint_var(0)
        event = StreamDataReceived(data=capsule_header, stream_id=CLIENT_BIDI_STREAM_ID, end_stream=False)
    ...
    async def test_push_promise_frame_is_ignored(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_PUSH_PROMISE, frame_data=b"\x01\x00")
    ...
    async def test_qpack_decompression_failed(
        ...
        mock_quic.close.assert_called_once_with(
            error_code=ErrorCodes.QPACK_DECOMPRESSION_FAILED,
            reason_phrase=expected_reason,
        )

    async def test_qpack_decoder_stream_error(
        self,
        mock_quic: MagicMock,
        mock_pylsqpack: dict[str, MagicMock],
        client_config: ClientConfig,
        ...
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)
        mock_pylsqpack["encoder_instance"].feed_decoder.side_effect = pylsqpack.DecoderStreamError("decoder error")
        data = encode_uint_var(constants.H3_STREAM_TYPE_QPACK_DECODER) + b"some data"
        event = StreamDataReceived(data=data, stream_id=SERVER_UNI_STREAM_ID, end_stream=False)
        ...
        mock_quic.close.assert_called_once_with(
            error_code=ErrorCodes.QPACK_DECODER_STREAM_ERROR, reason_phrase=expected_reason
        )
    ...
    async def test_unknown_frame_on_control_stream_is_ignored(
        ...
        unknown_frame = _encode_frame(frame_type=0x20, frame_data=b"test data")
...
class TestWebTransportH3EngineSending:
    def test_create_webtransport_stream_bidirectional(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        stream_id = engine.create_webtransport_stream(control_stream_id=123, is_unidirectional=False)

        assert stream_id == 0
        expected_payload = encode_uint_var(constants.H3_FRAME_TYPE_WEBTRANSPORT_STREAM) + encode_uint_var(123)
        mock_quic.send_stream_data.assert_any_call(stream_id=stream_id, data=expected_payload, end_stream=False)
    ...
    def test_create_webtransport_stream_unidirectional(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        stream_id = engine.create_webtransport_stream(control_stream_id=456, is_unidirectional=True)

        assert stream_id == 14
        mock_quic.send_stream_data.assert_any_call(stream_id=stream_id, data=encode_uint_var(456), end_stream=False)
    ...
    def test_send_capsule(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        mock_quic.send_stream_data.assert_any_call(stream_id=CLIENT_BIDI_STREAM_ID, data=capsule_data, end_stream=False)

    def test_send_capsule_on_uni_stream_fails(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        engine = WebTransportH3Engine(quic=mock_quic, config=client_config)

        with pytest.raises(
            ProtocolError,
            match="Capsules can only be sent on client-initiated bidirectional streams.",
        ) as excinfo:
            engine.send_capsule(stream_id=CLIENT_UNI_STREAM_ID, capsule_data=b"fail")
    ...
    def test_send_datagram_on_uni_stream_fails(self, mock_quic: MagicMock, client_config: ClientConfig) -> None:
        ...
        with pytest.raises(
            ProtocolError,
            match="Datagrams can only be sent for client-initiated bidirectional streams",
        ) as excinfo:
    ...
    def test_send_headers(
        ...
        expected_frame = _encode_frame(frame_type=constants.H3_FRAME_TYPE_HEADERS, frame_data=b"headers-payload")
