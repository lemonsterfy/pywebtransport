...
@pytest.fixture
def mock_quic_connection(mocker):
...
@pytest.fixture
def mock_parent_connection(mocker):
...
@pytest.fixture
def mock_h3_connection(mocker):
    return mocker.create_autospec(H3Connection, instance=True)


@pytest.fixture
def handler_client(mocker, mock_quic_connection, mock_parent_connection, mock_h3_connection):
...
@pytest.fixture
def handler_server(mocker, mock_quic_connection, mock_parent_connection, mock_h3_connection):
    mocker.patch("pywebtransport.protocol.handler.H3Connection", return_value=mock_h3_connection)
    handler = WebTransportProtocolHandler(
        quic_connection=mock_quic_connection,
        is_client=False,
...
class TestWebTransportProtocolHandler:
    def test_initialization_client(self, handler_client, mock_quic_connection):
    ...
    def test_initialization_server(self, handler_server):
        assert handler_server._is_client is False

    def test_connection_established(self, handler_client, mock_parent_connection):
    ...
    def test_connection_established_when_already_connected(self, handler_client):
        handler_client.connection_established()
        handler_client._stats["connected_at"] = 123.0
        handler_client.connection_established()
        assert handler_client._stats["connected_at"] == 123.0

    def test_properties(self, handler_client, mock_quic_connection):
        assert handler_client.quic_connection is mock_quic_connection
        assert isinstance(handler_client.stats, dict)

    def test_get_session_info_and_all_sessions(self, handler_client):
    ...
    def test_get_health_status(self, handler_client):
        handler_client.connection_established()
    ...
    def test_get_health_status_unhealthy(self, handler_client):
        health = handler_client.get_health_status()
    ...
    @pytest.mark.asyncio
    async def test_create_webtransport_session_server_raises_error(self, handler_server):
    ...
    @pytest.mark.asyncio
    async def test_create_session_with_no_server_name(self, handler_client, mock_quic_connection, mock_h3_connection):
    ...
    @pytest.mark.asyncio
    async def test_establish_session_not_connected(self, handler_client):
        with pytest.raises(ConnectionError):
            await handler_client.establish_session(path="/")

    @pytest.mark.asyncio
    async def test_establish_session_timeout(self, handler_client, mocker):
        handler_client.connection_established()
    ...
    @pytest.mark.asyncio
    async def test_recover_session_success(self, handler_client, mocker):
        ...
        assert await handler_client.recover_session(session_id="s1") is True
        assert sleep_mock.call_count == 1
        assert create_session_mock.call_count == 2

    @pytest.mark.asyncio
    async def test_recover_session_failure(self, handler_client, mocker):
        sleep_mock = mocker.patch("asyncio.sleep", new_callable=mocker.AsyncMock)
        create_session_mock = mocker.patch.object(
            handler_client, "create_webtransport_session", new_callable=mocker.AsyncMock
        )
    ...
    def test_accept_webtransport_session_server_success(self, handler_server, mock_h3_connection):
        session_info = WebTransportSessionInfo(
    ...
    def test_accept_webtransport_session_client_raises_error(self, handler_client):
        with pytest.raises(ProtocolError):
            handler_client.accept_webtransport_session(stream_id=0, session_id="any")

    def test_accept_webtransport_session_server_not_found_raises_error(self, handler_server):
        with pytest.raises(ProtocolError):
            handler_server.accept_webtransport_session(stream_id=0, session_id="none")

    @pytest.mark.asyncio
    async def test_close_webtransport_session(self, handler_client, mock_quic_connection):
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=4, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", session_info)
        future = asyncio.Future()
        handler_client.on(EventType.SESSION_CLOSED, future.set_result)
    ...
    def test_close_session_no_op(self, handler_client):
        handler_client.close_webtransport_session("non-existent-session")

    @pytest.mark.parametrize("is_unidirectional", [True, False])
    def test_create_webtransport_stream(self, handler_client, mock_h3_connection, is_unidirectional):
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", session_info)
    ...
    def test_create_stream_on_invalid_session_raises_error(self, handler_client):
        with pytest.raises(ProtocolError):
            handler_client.create_webtransport_stream("none")

    def test_send_webtransport_stream_data(self, handler_client, mock_h3_connection):
        s_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", s_info)
    ...
    def test_send_to_unwritable_stream_raises_error(self, handler_client):
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", session_info)
    ...
    @pytest.mark.asyncio
    async def test_read_and_write_stream(self, handler_client):
        handler_client.connection_established()
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
    ...
    def test_write_stream_chunked_with_error(self, handler_client, mocker):
        handler_client.connection_established()
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
    ...
    def test_abort_stream(self, handler_client, mock_quic_connection):
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", session_info)
    ...
    def test_send_webtransport_datagram(self, handler_client, mock_h3_connection):
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", session_info)
    ...
    @pytest.mark.asyncio
    async def test_handle_h3_event_routes_correctly(self, handler_client, mocker):
    ...
    @pytest.mark.asyncio
    async def test_handle_session_headers_client_success(self, handler_client):
        session_id, stream_id = await handler_client.create_webtransport_session(path="/test")
        future = asyncio.Future()
        handler_client.on(EventType.SESSION_READY, future.set_result)
        ...
        assert (await asyncio.wait_for(future, 1)).data["session_id"] == session_id

    @pytest.mark.asyncio
    async def test_handle_session_headers_client_failure(self, handler_client):
        session_id, stream_id = await handler_client.create_webtransport_session(path="/test")
        future = asyncio.Future()
        handler_client.on(EventType.SESSION_CLOSED, future.set_result)
        await handler_client._handle_session_headers(
    ...
    @pytest.mark.asyncio
    async def test_handle_session_headers_server_request(self, handler_server, mock_parent_connection):
        future = asyncio.Future()
        handler_server.on(EventType.SESSION_REQUEST, future.set_result)
    ...
    @pytest.mark.asyncio
    async def test_handle_webtransport_stream_data_new_stream(self, handler_server, mocker):
        s_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        ...
        handler_server.on(EventType.STREAM_OPENED, opened_future.set_result)
        await handler_server._handle_webtransport_stream_data(
            WebTransportStreamDataReceived(stream_id=5, data=b"", stream_ended=False, session_id=0)
        )
        assert (await asyncio.wait_for(opened_future, 1)).data["direction"] == StreamDirection.BIDIRECTIONAL

    @pytest.mark.asyncio
    async def test_handle_unhandled_h3_event(self, handler_client):
    ...
    @pytest.mark.asyncio
    async def test_handle_data_for_unknown_session(self, handler_server):
        handler_server._h3._get_or_create_stream.return_value = StreamInfo(
            session_id=0, stream_id=5, direction=StreamDirection.BIDIRECTIONAL, state=StreamState.OPEN, created_at=0
        )
        await handler_server._handle_webtransport_stream_data(
    ...
    @pytest.mark.asyncio
    async def test_handle_datagram_for_unknown_session(self, handler_server):
        await handler_server._handle_datagram_received(DatagramReceived(stream_id=0, data=b""))

    def test_stream_state_transitions_to_closed(self, handler_client):
        session_info = WebTransportSessionInfo(
            session_id="s1", stream_id=0, state=SessionState.CONNECTED, path="/", created_at=0
        )
        handler_client._register_session("s1", session_info)
